...
function TeachertProfile(props) {
    const AddAdditionalFormDisplay = useSelector(state => state.AddAdditionalFormDisplay)
    ...
    const removeAdditionalById = async (id) => {
        ...
        await axios.delete('http://localhost:3333/api/additional', {
            data: {
                id
            }
        })
    ...
    return (
        <div>
            ...
            <h3>{props.rank}</h3>
            <h2>Допы</h2>
            {!AddAdditionalFormDisplay && <button onClick={() => dispatch(switchFormDisplay())}>+</button>}
            {AddAdditionalFormDisplay && <AddAdditionalForm subjects={props.subjects} />}
